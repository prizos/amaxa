...
Tried again and still not readable from here. Diodes Incorporated's own
datasheet (DS37841 Rev. 3-2, November 2015) was fetched and its text extracted:
under Mechanical Data it says *"Terminals Connections: See Diagram Below"*, and
that diagram carries no text layer — the only images in the file are a package
photograph and a lead-free logo, so the pin letters are drawn as vector paths.
The manufacturer's part page returns 403 to anything without a browser, and
this environment has no PDF renderer (`make offline` is why it has no package
manager to install one either). So the check is a person opening the PDF at
page 1 and looking: a minute's work that cannot be automated from here.
