...
def tracks_of(pcb_text: str, nets: tuple[str, ...], layer: str = "F.Cu") -> dict[str, list]:
    """net -> [((x1, y1), (x2, y2), width), ...] for its segments on one layer."""
    ...
    for block in re.findall(r"\n\t\(segment\n(?:\t\t[^\n]*\n)+\t\)", pcb_text):
        ...
        if not all(found.values()):
            continue
        if found["layer"].group(1) != layer or found["net"].group(1) not in wanted:
            continue
...
def controlled_width(tracks: dict[str, list]) -> float:
    ...
    return max(round(width, 4) for segments in tracks.values() for *_, width in segments)
