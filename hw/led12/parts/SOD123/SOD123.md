...
**Needs a human eye.** The banded-end convention is read from the KiCad
footprint's silkscreen and the standard SOD-123 datasheet convention, not from
this manufacturer's own package drawing. Fitted backwards the Zener becomes a
forward diode from gate to source, which would hold the FET off and leave the
board dead rather than unprotected — a loud failure, not a silent one, but
