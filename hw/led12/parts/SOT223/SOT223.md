...
**19.9 V**. During a surge the regulator would have failed before the TVS did
...
conducting cannot clamp below 15 V at rated surge current, so the regulator had
...
**Needs a human eye.** The package-outline page is a raster image, so the tab
lead's bonding could not be read from the datasheet text. In a 3-lead SOT-223
the tab is the same piece of leadframe as the centre pin — which is why KiCad
calls the footprint `SOT-223-3_TabPin2` — and LCSC's own footprint numbers the
tab pad `2`, so tab = VOUT. Both point the same way, but if the tab were ground
it would short 3V3 to GND on this board, so confirm it on page 9 of the PDF
before release.
...
**24 V** — clears the 19.9 V clamp, but by 4 V rather than 24 V, and its
quiescent current is 2 mA typical. Worth taking if the Gainsil part's stock of
585 runs out or the tab check above fails.
